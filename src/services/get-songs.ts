...
export default class {
    ...
    try {
      ...
      if (YOUTUBE_HOSTS.includes(url.host)) {
        ...
        const [convertedSongs, nSongsNotFound, totalSongs] =
          await this.spotifySource(query, playlistLimit, shouldSplitChapters)
  ...
  private async spotifySource(
    ...
    switch (parsed.type) {
      case 'album': {
        const [tracks, playlist] = await this.spotifyAPI.getAlbum(
          url,
          playlistLimit,
        )
      ...
      case 'playlist': {
        const [tracks, playlist] = await this.spotifyAPI.getPlaylist(
          url,
          playlistLimit,
        )
        return this.spotifyToYouTube(tracks, shouldSplitChapters, playlist)
